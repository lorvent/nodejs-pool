...
let paymentQueue = async.queue(function (paymentDetails, callback) {
    ...
    global.support.rpcWallet(transferFunc, paymentDetails, function (body) {
        ...
        if (body.hasOwnProperty('error')) {
            if (body.error.message === "Transaction would be too large.  try /transfer_split."){
                console.error("Issue making payments, Transaction would be too large. , will try later");
                if(!extraPaymentRound){
                    setTimeout(function(){
                        makePayments();
                    }, global.config.payout.timerRetry * 60 * 1000 * 2);
                }
    ...
    global.mysql.query("SELECT * FROM balance WHERE amount >= ?", [global.support.decimalToCoin(global.config.payout.walletMin)]).then(function (rows) {
        ...
        rows.forEach(function (row) {
            ...
            global.mysql.query("SELECT payout_threshold FROM users WHERE username = ?", [payee.id]).then(function (userRow) {
				...
				//dero features max 29 coin
				if (payee.amount > global.support.decimalToCoin(29)) {
					console.log("Big pay to: " + payee.address + "Payee balance: " + global.support.coinToDecimal(payee.amount));
					payee.amount = global.support.decimalToCoin(29);
					} 
                ...
                if (roundCount === rows.length && paymentDestinations.length > 0) {
					...
					console.log("------------------");
					paymentDestinations.sort(function (a, b) {
